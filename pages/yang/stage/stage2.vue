...
function init() {
    moveData.value = [];
    tempData.value = [];
	// 一共需要72*4=288张；  288/16=18，16种，每种18张
    // 创建一个长度为 18 * simpleData.value.length = 288的数组，用于生成初始卡片组的数量
...
function checkDisabled() {
    cellHtml.value.forEach((v) => {
        ...
        const currentLayer = v.ly; // 当前层级
        const currentColumn = arr[2]; // 当前列号
        let isBlocked = false; // 默认不被遮挡

        // for (let i = currentLayer + 1; i <= layerCount.value - 4; i++) {
        //     // 检查同一列上是否有更高层级的卡片
        //     if (cellHtml.value.some(item => {
        //         const itemArr = item.id.substring(1).split('-').map(Number);
        //         return item.j === currentColumn && itemArr[0] === i;
        //     })) {
        //         isBlocked = true;
        //         break;
        //     }
...
// 检测 每列临时区的三个是否为空，若为空则将该列往下移动三个元素
function checkAndMoveTemp() {
    const maxRows = 3; // 临时区域最大行数
    const maxCols = 4; // 临时区域最大列数

    // 遍历每一列
    for (let col = 0; col < maxCols; col++) {
        // 获取当前列的所有元素
        const colItems = cellHtml.value.filter(item => {
            const itemCol = item.j;
            return itemCol === col;
        });
        ...
        if (tempItems.length === 0 && normalItems.length >= 4) {
            ...
            lastFour.forEach((item, index) => {
                item.ly = 999; // 更新为临时区域标识
                item.disabled = false;
                item.top = maxTop + (index * 100); // 设置新的位置，每个相差100
                // xiayiCardCount.value = xiayiCardCount.value + 4;
            });
        }
    }
}
